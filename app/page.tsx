          ...
          >
            ...
            <div className="flex flex-col gap-2">
              <h3 className="text-xl font-semibold">
                <Link className="text-slate-900 hover:text-blue-700" href={`/episodes/${ep.slug}`}>
                  {ep.title}
                </Link>
              </h3>
              <div className="flex flex-wrap items-center gap-2 text-sm">
                <span className="inline-flex items-center gap-1 rounded-full bg-slate-100 px-2 py-0.5 text-slate-700">📅 {new Date(ep.date).toLocaleDateString()}</span>
                <span className="inline-flex items-center gap-1 rounded-full bg-slate-100 px-2 py-0.5 text-slate-700">⏱️ {ep.duration}</span>
              </div>
